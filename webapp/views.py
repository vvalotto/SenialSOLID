...
def versiones():
    lista_componentes = []
    lista_componentes.append('Adquisidor: ' + adquisidor.__version__)
    lista_componentes.append('Procesador: ' + procesador.__version__)
    lista_componentes.append('Persistidor: ' + persistidor.__version__)
...
@app.route('/acerca/')
def acerca():
    return render_template('/templates/general/acerca.html')
...
def adquisicion():
    ...
    if form.validate_on_submit():
        ...
        sa.id = form.identificador
        sa.comentario = form.descripcion
        sa.fecha_adquisciom = form.fecha
        ra.guardar(sa)
        return redirect(url_for('aplicacion/adquisicion.html'))
